...
public class MonitorBoardWithThread {
	...
	private Thread depthsDisThread = null;
	private Thread chlCntDisThread = null;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					MonitorBoardWithThread window = new MonitorBoardWithThread();
	...
	private void initialize() {
		...
		btnNewButton = new JButton("QDepthMonitor");
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(null==depthsDisThread){
					depthsDisThread = new Thread(new Runnable(){
						public void run(){
							try {
					...
					depthsDisThread.start();
					if(null!=btnNewButton)
						btnNewButton.setText("StopDepthMonitor");					
					//TODO 不然的话需要一些错误处理						
				} else {
					depthsDisThread.interrupt();
					depthsDisThread = null;
					btnNewButton.setText("QDepthMonitor");
				}
